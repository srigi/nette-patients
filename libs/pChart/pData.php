...
/*
 * Data - Simplifying data population for pChart
 * opyright (C) 2008 Jean-Damien POGOLOTTI
 * ersion  1.13 last updated on 08/17/08
 ...
 * lass initialisation :
 * Data()
 * ata populating methods :
 * mportFromCSV($FileName,$Delimiter=",",$DataColumns=-1,$HasHeader=FALSE,$DataName=-1)
 * ddPoint($Value,$Serie="Serie1",$Description="")
 * eries manipulation methods :
 * ddSerie($SerieName="Serie1")
 * ddAllSeries()
 * emoveSerie($SerieName="Serie1")
 * etAbsciseLabelSerie($SerieName = "Name")
 * etSerieName($Name,$SerieName="Serie1")
 +   SetSerieSymbol($Name,$Symbol)
 * etXAxisName($Name="X Axis")
 * etYAxisName($Name="Y Axis")
 * etXAxisFormat($Format="number")
 * etYAxisFormat($Format="number")
 * etXAxisUnit($Unit="")
 * etYAxisUnit($Unit="")
 * emoveSerieName($SerieName)
 * emoveAllSeries()
 * ata retrieval methods :
 * etData()
 * etDataDescription()
 */
            ...
            while (!feof($handle)) {
                ...
                if ($buffer != "") {
                    if ($HasHeader == TRUE && $HeaderParsed == FALSE) {
                        if ($DataColumns == - 1) {
                            $ID = 1;
                            foreach ($Values as $key => $Value) { $this->SetSerieName($Value, "Serie" . $ID); $ID++;                            }
                        } else {
                            $SerieName = "";
                        ...
                        if ($DataColumns == - 1) {
                            $ID = 1;
                            foreach ($Values as $key => $Value) { $this->AddPoint(intval($Value), "Serie" . $ID); $ID++;                            }
                        } else {
                            $SerieName = "";
                            if ($DataName != - 1)
