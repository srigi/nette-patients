...
/*
 * Chart - a PHP class to build charts!
 * opyright (C) 2008 Jean-Damien POGOLOTTI
 * ersion  1.27d last updated on 09/30/08
 * ttp://pchart.sourceforge.net
 * his program is free software: you can redistribute it and/or modify
 * t under the terms of the GNU General Public License as published by
 * he Free Software Foundation, either version 1,2,3 of the License, or
 ...
 * raw methods :
 * rawBackground($R,$G,$B)
 * rawRectangle($X1,$Y1,$X2,$Y2,$R,$G,$B)
 * rawFilledRectangle($X1,$Y1,$X2,$Y2,$R,$G,$B,$DrawBorder=TRUE,$Alpha=100)
 * rawRoundedRectangle($X1,$Y1,$X2,$Y2,$Radius,$R,$G,$B)
 * rawFilledRoundedRectangle($X1,$Y1,$X2,$Y2,$Radius,$R,$G,$B)
 * rawCircle($Xc,$Yc,$Height,$R,$G,$B,$Width=0)
 * rawFilledCircle($Xc,$Yc,$Height,$R,$G,$B,$Width=0)
 * rawEllipse($Xc,$Yc,$Height,$Width,$R,$G,$B)
 * rawFilledEllipse($Xc,$Yc,$Height,$Width,$R,$G,$B)
 * rawLine($X1,$Y1,$X2,$Y2,$R,$G,$B,$GraphFunction=FALSE)
 * rawDottedLine($X1,$Y1,$X2,$Y2,$DotSize,$R,$G,$B)
 * rawAlphaPixel($X,$Y,$Alpha,$R,$G,$B)
 * rawFromPNG($FileName,$X,$Y,$Alpha=100)
 * rawFromGIF($FileName,$X,$Y,$Alpha=100)
 * rawFromJPG($FileName,$X,$Y,$Alpha=100)
 * raph setup methods :
 * ddBorder($Width=3,$R=0,$G=0,$B=0)
 * learScale()
 * learShadow()
 * reateColorGradientPalette($R1,$G1,$B1,$R2,$G2,$B2,$Shades)
 * rawGraphArea($R,$G,$B,$Stripe=FALSE)
 * rawScale($Data,$DataDescription,$ScaleMode,$R,$G,$B,$DrawTicks=TRUE,$Angle=0,$Decimals=1,$WithMargin=FALSE,$SkipLabels=1,$RightScale=FALSE)
 * rawRightScale($Data,$DataDescription,$ScaleMode,$R,$G,$B,$DrawTicks=TRUE,$Angle=0,$Decimals=1,$WithMargin=FALSE,$SkipLabels=1)
 * rawXYScale($Data,$DataDescription,$YSerieName,$XSerieName,$R,$G,$B,$WithMargin=0,$Angle=0,$Decimals=1)
 * rawGrid($LineWidth,$Mosaic=TRUE,$R=220,$G=220,$B=220,$Alpha=100)
 * rawLegend($XPos,$YPos,$DataDescription,$R,$G,$B,$Rs=-1,$Gs=-1,$Bs=-1,$Rt=0,$Gt=0,$Bt=0,$Border=FALSE)
 * rawPieLegend($XPos,$YPos,$Data,$DataDescription,$R,$G,$B)
 * rawTitle($XPos,$YPos,$Value,$R,$G,$B,$XPos2=-1,$YPos2=-1,$Shadow=FALSE)
 * rawTreshold($Value,$R,$G,$B,$ShowLabel=FALSE,$ShowOnRight=FALSE,$TickWidth=4,$FreeText=NULL)
 * rawArea($Data,$Serie1,$Serie2,$R,$G,$B,$Alpha = 50)
 * rawRadarAxis($Data,$DataDescription,$Mosaic=TRUE,$BorderOffset=10,$A_R=60,$A_G=60,$A_B=60,$S_R=200,$S_G=200,$S_B=200,$MaxValue=-1)
 * rawGraphAreaGradient($R,$G,$B,$Decay,$Target=TARGET_GRAPHAREA)
 * rawTextBox($X1,$Y1,$X2,$Y2,$Text,$Angle=0,$R=255,$G=255,$B=255,$Align=ALIGN_LEFT,$Shadow=TRUE,$BgR=-1,$BgG=-1,$BgB=-1,$Alpha=100)
 * etLegendBoxSize($DataDescription)
 * oadColorPalette($FileName,$Delimiter=",")
 * eportWarnings($Interface="CLI")
 * etGraphArea($X1,$Y1,$X2,$Y2)
 * etLabel($Data,$DataDescription,$SerieName,$ValueName,$Caption,$R=210,$G=210,$B=210)
 * etColorPalette($ID,$R,$G,$B)
 * etCurrency($Currency)
 * etDateFormat($Format)
 * etFontProperties($FontName,$FontSize)
 * etLineStyle($Width=1,$DotSize=0)
 * etFixedScale($VMin,$VMax,$Divisions=5,$VXMin=0,$VXMin=0,$XDivisions=5)
 * etShadowProperties($XDistance=1,$YDistance=1,$R=60,$G=60,$B=60,$Alpha)
 * riteValues($Data,$DataDescription,$Series)
 * raphs methods :
 * rawPlotGraph($Data,$DataDescription,$BigRadius=5,$SmallRadius=2,$R2=-1,$G2=-1,$B2=-1,$Shadow=FALSE)
 * rawXYPlotGraph($Data,$DataDescription,$YSerieName,$XSerieName,$PaletteID=0,$BigRadius=5,$SmallRadius=2,$R2=-1,$G2=-1,$B2=-1)
 * rawLineGraph($Data,$DataDescription,$SerieName="")
 * rawXYGraph($Data,$DataDescription,$YSerieName,$XSerieName,$PaletteID=0)
 * rawFilledLineGraph($Data,$DataDescription,$Alpha=100,$AroundZero=FALSE)
 * rawCubicCurve($Data,$DataDescription,$Accuracy=.1,$SerieName="")
 * rawFilledCubicCurve($Data,$DataDescription,$Accuracy=.1,$Alpha=100,$AroundZero=FALSE)
 * rawOverlayBarGraph($Data,$DataDescription,$Alpha=50)
 * rawBarGraph($Data,$DataDescription,$Shadow=FALSE)
 * rawStackedBarGraph($Data,$DataDescription,$Alpha=50,$Contiguous=FALSE)
 * rawLimitsGraph($Data,$DataDescription,$R=0,$G=0,$B=0)
 * rawRadar($Data,$DataDescription,$BorderOffset=10,$MaxValue=-1)
 * rawFilledRadar($Data,$DataDescription,$Alpha=50,$BorderOffset=10,$MaxValue=-1)
 * rawBasicPieGraph($Data,$DataDescription,$XPos,$YPos,$Radius=100,$DrawLabels=PIE_NOLABEL,$R=255,$G=255,$B=255,$Decimals=0)
 * rawFlatPieGraph($Data,$DataDescription,$XPos,$YPos,$Radius=100,$DrawLabels=PIE_NOLABEL,$SpliceDistance=0,$Decimals = 0)
 * rawFlatPieGraphWithShadow($Data,$DataDescription,$XPos,$YPos,$Radius=100,$DrawLabels=PIE_NOLABEL,$SpliceDistance=0,$Decimals = 0)
 * rawPieGraph($Data,$DataDescription,$XPos,$YPos,$Radius=100,$DrawLabels=PIE_NOLABEL,$EnhanceColors=TRUE,$Skew=60,$SpliceHeight=20,$SpliceDistance=0,$Decimals=0)
 * ther methods :
 * etImageMap($Mode=TRUE,$GraphID="MyGraph")
 * etImageMap($MapName,$Flush=TRUE)
 * ender($FileName)
 * troke()
 */
...
// * Fixed in 1.27 *         { $X2 = $this->GArea_X2 - 1; $Y2 = $this->GArea_Y2 - ($this->GArea_X2 - $X1); }
    ...
    function drawScale($Data, $DataDescription, $ScaleMode, $R, $G, $B, $DrawTicks=TRUE, $Angle=0, $Decimals=1, $WithMargin=FALSE, $SkipLabels=1, $RightScale=FALSE)
    {
        ...
        if ($this->VMin == NULL && $this->VMax == NULL) {
            ...
            if ($this->VMax == $this->VMin) {
                if ($this->VMax >= 0) { $this->VMax++;                } else { $this->VMin--;                }
            }
            ...
            if ($this->VMin == 0 && $this->VMax == 0) { $this->VMin = 0; $this->VMax = 2; $Scale = 1; $Divisions = 2;            } elseif ($MaxDivs > 1) {
                while (!$ScaleOk) {
    ...
    function drawXYScale($Data, $DataDescription, $YSerieName, $XSerieName, $R, $G, $B, $WithMargin=0, $Angle=0, $Decimals=1)
    {
        ...
        if ($this->VXMin == NULL && $this->VXMax == NULL) {
            ...
            if ($this->VXMin == 0 && $this->VXMax == 0) { $this->VXMin = 0; $this->VXMax = 2; $Scale = 1; $XDivisions = 2;            } elseif ($MaxDivs > 1) {
                while (!$ScaleOk) {
    ...
    function drawGrid($LineWidth, $Mosaic=TRUE, $R=220, $G=220, $B=220, $Alpha=100)
    {
        ...
        /* Vertical lines */
        if ($this->GAreaXOffset == 0) { $XPos = $this->GArea_X1 + $this->DivisionWidth + $this->GAreaXOffset; $ColCount = $this->DataCount - 2;        } else { $XPos = $this->GArea_X1 + $this->GAreaXOffset; $ColCount = floor(($this->GArea_X2 - $this->GArea_X1) / $this->DivisionWidth);        }
    ...
    function drawLegend($XPos, $YPos, $DataDescription, $R, $G, $B, $Rs=-1, $Gs=-1, $Bs=-1, $Rt=0, $Gt=0, $Bt=0, $Border=TRUE)
    {
        ...
        if ($Rs == - 1 || $Gs == - 1 || $Bs == - 1) { $Rs = $R - 30; $Gs = $G - 30; $Bs = $B - 30;        }
    ...
    function drawTreshold($Value, $R, $G, $B, $ShowLabel=FALSE, $ShowOnRight=FALSE, $TickWidth=4, $FreeText=NULL)
    {
        ...
        if ($ShowLabel) {
            if ($FreeText == NULL) { $Label = $Value;            } else { $Label = $FreeText;            }
    ...
    function setLabel($Data, $DataDescription, $SerieName, $ValueName, $Caption, $R=210, $G=210, $B=210)
    {
        ...
        $Cp = 0; $Found = FALSE;
        foreach ($Data as $Key => $Value) {
            if ($Data[$Key][$DataDescription["Position"]] == $ValueName) { $NumericalValue = $Data[$Key][$SerieName]; $Found = TRUE;            }
            if (!$Found)
                $Cp++;
        }
    ...
    function drawPlotGraph($Data, $DataDescription, $BigRadius=5, $SmallRadius=2, $R2=-1, $G2=-1, $B2=-1, $Shadow=FALSE)
    {
        ...
        foreach ($DataDescription["Values"] as $Key2 => $ColName) {
            $ID = 0;
            foreach ($DataDescription["Description"] as $keyI => $ValueI) { if ($keyI == $ColName) { $ColorID = $ID;                }; $ID++;            }
        ...
        foreach ($DataDescription["Values"] as $Key2 => $ColName) {
            ...
            foreach ($Data as $Key => $Values) {
                ...
                if (!is_numeric($Value)) {
                    ...
                    $PointsCount++;
                    if ($YLast <> $Empty) { $aPoints[] = $XPos; $aPoints[] = $YPos;                    } else { $PointsCount++; $aPoints[] = $XPos; $aPoints[] = $LayerHeight; $aPoints[] = $XPos; $aPoints[] = $YPos;                    }
    ...
    function drawLimitsGraph($Data, $DataDescription, $R=0, $G=0, $B=0)
    {
        ...
        foreach ($Data as $Key => $Values) {
            ...
            foreach ($DataDescription["Values"] as $Key2 => $ColName) {
                if (isset($Data[$Key][$ColName])) {
                    if ($Data[$Key][$ColName] > $Max && is_numeric($Data[$Key][$ColName])) { $Max = $Data[$Key][$ColName]; $MaxID = $GraphID;                    }
                }
                if (isset($Data[$Key][$ColName]) && is_numeric($Data[$Key][$ColName])) {
                    if ($Data[$Key][$ColName] < $Min) { $Min = $Data[$Key][$ColName]; $MinID = $GraphID;                    }
                    $GraphID++;
                }
            }
    ...
    function drawRadarAxis($Data, $DataDescription, $Mosaic=TRUE, $BorderOffset=10, $A_R=60, $A_G=60, $A_B=60, $S_R=200, $S_G=200, $S_B=200, $MaxValue=-1)
    {
        ...
        for ($i = 0; $i<=$Points; $i++) {
            ...
            if (isset($Data[$i][$DataDescription["Position"]])) {
                ...
                if ($Angle > 90 && $Angle <= 180) { $YOffset = $Height; $XOffset = -$Width;                }

                if ($Angle > 180 && $Angle <= 270) { $XOffset = -$Width;                }
        ...
        foreach ($DataDescription["Values"] as $Key2 => $ColName) {
            ...
            foreach ($Data as $Key => $Values) {
                if (isset($Data[$Key][$ColName])) {
                    ...
                    if ($XLast == - 1) { $FirstX = $XPos; $FirstY = $YPos;                    }
    ...
    function drawBasicPieGraph($Data, $DataDescription, $XPos, $YPos, $Radius=100, $DrawLabels=PIE_NOLABEL, $R=255, $G=255, $B=255, $Decimals=0)
    {
        ...
        foreach ($TopPlots as $Key => $Value) { foreach ($TopPlots[$Key] as $Key2 => $Value2) { settype($TopPlots[$Key][$Key2], "float");            }        }
    ...
    function drawFlatPieGraph($Data, $DataDescription, $XPos, $YPos, $Radius=100, $DrawLabels=PIE_NOLABEL, $SpliceDistance=0, $Decimals=0, $AllBlack=FALSE)
    {
        ...
        foreach ($iValues as $Key => $Value) {
            ...
            if ($AllBlack) { $Rc = $this->ShadowRColor; $Gc = $this->ShadowGColor; $Bc = $this->ShadowBColor;            } else { $Rc = $this->Palette[$Key]["R"]; $Gc = $this->Palette[$Key]["G"]; $Bc = $this->Palette[$Key]["B"];            }
    ...
    function drawPieGraph($Data, $DataDescription, $XPos, $YPos, $Radius=100, $DrawLabels=PIE_NOLABEL, $EnhanceColors=TRUE, $Skew=60, $SpliceHeight=20, $SpliceDistance=0, $Decimals=0)
    {
        ...
        foreach ($DataDescription["Values"] as $Key2 => $ColName) {
            if ($ColName != $DataDescription["Position"]) {
                ...
                foreach ($Data as $Key => $Values)
                    if (isset($Data[$Key][$ColName])) {
                        if ($Data[$Key][$ColName] == 0) { $iValues[] = 0; $rValues[] = 0; $iLabels[] = $Data[$Key][$DataDescription["Position"]];                        }
                        // Removed : $PieSum++; $rValues[] = 1;
                        else { $PieSum += $Data[$Key][$ColName]; $iValues[] = $Data[$Key][$ColName]; $iLabels[] = $Data[$Key][$DataDescription["Position"]]; $rValues[] = $Data[$Key][$ColName]; $rPieSum += $Data[$Key][$ColName];                        }
                    }
            }
        }
