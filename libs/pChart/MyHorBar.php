 ...
 * xtension by Gabriele FERRI
 * ttp://www.gabrieleferri.it
 * yHorBar.class is writen to extend pChart.class to write a horizontal bar charts
 * opyright (C) 2009 Gabriele FERRI
 * ersion 1.0beta 08/05/2009
 * ontact me with bug reports and comments: info@gabrieleferri.it
 ...
 * raw methods :
 * rawHorBarGraph($Data,$DataDescription,$Shadow=TRUE,$Alpha=100)
 * rawHorScale($Data,$DataDescription,$ScaleMode,$R,$G,$B,$DrawTicks=TRUE,$Angle=0,$Decimals=1,$WithMargin=FALSE,$SkipLabels=1,$RightScale=FALSE)
 * rawHorGrid($LineWidth,$Mosaic,$R=220,$G=220,$B=220,$Alpha=100)
 * rawTreshold($Value,$R,$G,$B,$ShowLabel=FALSE,$ShowOnBottom=FALSE,$TickWidth=4,$FreeText=NULL,$Angle=0)
 */
...
class MyHorBar extends pChart
{
    ...
    function drawHorGrid($LineWidth, $Mosaic=TRUE, $R=220, $G=220, $B=220, $Alpha=100)
    {
        ...
        /* Horizontal lines */
        if ($this->GAreaYOffset == 0) { $YPos = $this->GArea_Y1 + $this->DivisionWidth + $this->GAreaYOffset;
            $ColCount = $this->DataCount - 2;        } else { $YPos = $this->GArea_Y1 + $this->GAreaYOffset;
